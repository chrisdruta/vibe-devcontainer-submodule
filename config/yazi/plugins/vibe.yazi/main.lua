...
-- The repaint entry point was renamed across yazi versions; without one the
-- badge still appears on the next UI event.
local function refresh()
	local fn = ya.render or ya.redraw
	if fn then
		fn()
	end
end
...
local set_many = ya.sync(function(_, found)
	for p, v in pairs(found) do
		verdicts[p] = v
	end
	refresh()
end)
...
local first_visit = ya.sync(function(_, cwd)
...
-- wins — the same rule consuming agents apply.
local function load_existing(cwd)
...
function M:entry(job)
	...
	if first_visit(st.cwd) then
		load_existing(st.cwd)
	end
