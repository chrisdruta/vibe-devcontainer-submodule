...
- **No root, no sudo.** The container runs as `vscode` with all capabilities
  ...
  by a rebuild (`./.devcontainer/vibe rebuild`, run on the HOST, not in here).
- **Never source `.env` into a shell.** Secrets load per-process only:
  `./.devcontainer/vibe agent` / `vibe run CMD` on the host, or
...
- **Edits to `devcontainer.json` or the Dockerfile do nothing until
  `vibe rebuild`** — and that must run on the host. Say so instead of retrying
  in-container.
