...
#
# Config resolution: a project-owned .devcontainer/yazi/ (seeded from
# templates/yazi with the a/r verdict keybindings -> vibe-verdict) wins;
# otherwise the harness's plain browsing config. Verdict recording therefore
# lives entirely in project config, not in harness code.
#
...
# keybindings, and the badge linemode; a project's .devcontainer/yazi/
...
resolve_config_home() {
  ...
  while [ "$dir" != "/" ]; do
    if [ -d "$dir/.devcontainer/yazi" ] || [ -f "$dir/.devcontainer/devcontainer.json" ]; then
      [ -d "$dir/.devcontainer/yazi" ] && proj="$dir/.devcontainer/yazi"
      if [ -f "$dir/.devcontainer/config.env" ]; then
        set -a
        # shellcheck disable=SC1091
        . "$dir/.devcontainer/config.env"
        set +a
      fi
      break
    fi
