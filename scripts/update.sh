...
# container (agents: `bash .devcontainer/harness/scripts/update.sh`) — only
...
sub="$repo_root/.devcontainer/harness"
if ! git -C "$sub" rev-parse --git-dir >/dev/null 2>&1; then
  echo "No harness submodule at $sub" >&2
  echo "Initialize it first: git submodule update --init .devcontainer/harness" >&2
  exit 1
fi
...
if ! new_sha="$(git -C "$sub" rev-parse -q --verify "$target^{commit}")"; then
  ...
  echo "Available: git -C .devcontainer/harness tag --list --sort=-v:refname" >&2
  exit 1
fi
...
git -C "$repo_root" add .devcontainer/harness

changed="$(git -C "$sub" diff --name-only "$old_sha" "$new_sha" --)"
rebuild=""
if printf '%s\n' "$changed" | grep -q '^Dockerfile$'; then
  rebuild=required
elif printf '%s\n' "$changed" | grep -qE '^(scripts/|config/)'; then
  rebuild=recommended
fi
templates_changed=""
if printf '%s\n' "$changed" | grep -q '^templates/'; then
  templates_changed=1
fi
in_container=""
if [ -e /.dockerenv ]; then
...
echo "  git commit -m \"Update devcontainer harness to $target\""
echo "  ./.devcontainer/vibe doctor"
...
if [ "$rebuild" = "required" ]; then
  ...
  echo "  ./.devcontainer/vibe rebuild"
